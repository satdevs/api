...
?>
		<div class="index col-12">
            <div class="card card-lightblue">
				...
				<?php //= __('Templates') ?>
				<div class="card-body table-responsive p-0 templates">
...
<?php } ?>
					...
					<?php foreach ($templates as $template): ?>
							<tr row-id="<?= $template->id ?>"<?php if($template->id == $layoutTemplatesLastId){ echo ' class="table-tr-last-id"'; } ?>  prefix="<?= $prefix ?>" controller="<?= $controller ?>" action="<?= $action ?>" aria-expanded="true">
								<td class="row-id-anchor" value="<?= $template->id ?>"><a class="anchor" name="<?= $template->id ?>"></a></td><!-- bake-0 -->
<?php if(isset($config['index_show_id']) && $config['index_show_id']){ ?>
								<td class="id integer" name="id" value="<?= $this->Number->format($template->id) ?>"><?= $this->Number->format($template->id) ?></td><!-- bake-3 -->
<?php } ?>
								<td class="name string" name="name" value="<?= $template->name ?>"><?= h($template->name) ?></td><!-- bake-2 -->
								<td class="title string" name="title" value="<?= $template->title ?>"><?= h($template->title) ?></td><!-- bake-2 -->
...
<?php 	if(isset($config['index_enable_view']) && $config['index_enable_view']){ ?>
									<?= $this->Html->link('<i class="fas fa-eye"></i>', ['action' => 'view', $template->id], ['escape' => false, 'class' => 'btn btn-sm bg-gradient-warning action-button-view', 'data-bs-tooltip'=>'tooltip', 'data-bs-placement'=>'top', 'title' => __('View this record')]) ?>
<?php 	} ?>
<?php 	if(isset($config['index_enable_edit']) && $config['index_enable_edit']){ ?>
									<?= $this->Html->link('<i class="fas fa-edit"></i>', ['action' => 'edit', $template->id], ['escape' => false, 'class' => 'btn btn-sm bg-gradient-success action-button-edit', 'data-bs-tooltip'=>'tooltip', 'data-bs-placement'=>'top', 'title' => __('Edit this record')]) ?>
<?php 	} ?>
<?php 	if(isset($config['index_enable_delete']) && $config['index_enable_delete']){ ?>
									...
									<?= $this->Form->postLink('', ['action' => 'delete', $template->id], ['class'=>'crose-btn hide-postlink action-button-delete']) ?>
									<a href="javascript:;" class="btn btn-sm btn-danger delete postlink-delete" data-bs-tooltip="tooltip" data-bs-placement="top" title="<?= __("Delete this record!") ?>" text="<?= h($template->name) ?>" subText="<?= __("You will not be able to revert this!") ?>" confirmButtonText="<?= __("Yes, delete it!") ?>" cancelButtonText="<?= __("Cancel") ?>"><i class="icon-minus"></i></a>
...
<?php $url = $this->Url->build(['controller' => 'Templates', 'action' => $config['index_db_click_action']]); ?>
		$('tr').dblclick( function(){
<?php /* window.location.href = '/<?= $prefix ?>/templates/<?= $config['index_db_click_action'] ?>/'+$(this).attr('row-id'); */ ?>
...
<?php if(isset($controller)){ ?>
		$(".pagination a[href^='<?= $base ?>/<?= $prefix ?>/templates?sort=']").each(function(){
			this.href = this.href.replace("<?= $base ?>/<?= $prefix ?>/templates?sort=", "<?= $base ?>/<?= $prefix ?>/templates?page=1&sort=");
		});
		$(".pagination a[href='<?= $base ?>/<?= $prefix ?>/templates']").each(function(){
			this.href = this.href.replace("<?= $base ?>/<?= $prefix ?>/templates", "<?= $base ?>/<?= $prefix ?>/templates?page=1");
		});
<?php } ?>
